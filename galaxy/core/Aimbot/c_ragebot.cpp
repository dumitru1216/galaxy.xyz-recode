...
Vector c_aimbot::Hitscan( C_BaseEntity* pEnt ) // supremeemmemememememe
{
	...
	if (!galaxy_vars.cfg.DelayShot && Velocity > 29.f)
		Velocity = 30.f;
...
void c_aimbot::OnCreateMove( )
{
	...
	for (int i = 1; i <= g_pEngine->GetMaxClients( ); ++i)
	{
		...
		if (g_LagComp.PlayerRecord[i].size( ) == 0 || !g::pLocalEntity->IsAlive( ) || !galaxy_vars.cfg.Aimbot || galaxy_vars.cfg.LegitBacktrack)
			continue;

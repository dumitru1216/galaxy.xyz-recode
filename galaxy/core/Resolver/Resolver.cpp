...
void Resolver::AnimationFix(C_BaseEntity* pEnt)
{
	...
	if (storedSimtime[pEnt->EntIndex()] != pEnt->GetSimulationTime())
	{
		...
		if (pEnt != g::pLocalEntity && pEnt->GetTeam() != g::pLocalEntity->GetTeam() && (pEnt->GetFlags() & FL_ONGROUND) && galaxy_vars.cfg.Resolver)
		{
	...
	if (pEnt != g::pLocalEntity && pEnt->GetTeam() != g::pLocalEntity->GetTeam() && (pEnt->GetFlags() & FL_ONGROUND) && galaxy_vars.cfg.Resolver && jittering[pEnt->EntIndex()])
